...
module "ecs" {
  ...
  restrict_to_cloudfront     = false # CloudFront prefix list exceeds SG rule limit; using admin secret header instead
...
# -----------------------------------------------------------------------------
# Admin Secret for CloudFront
# -----------------------------------------------------------------------------

data "aws_secretsmanager_secret_version" "admin_header" {
  secret_id = "kong-llm-gateway-poc/admin-header-secret"
}
...
module "cloudfront" {
  ...
  # Admin secret header for bypassing admin endpoint blocking
  admin_secret_header = data.aws_secretsmanager_secret_version.admin_header.secret_string
